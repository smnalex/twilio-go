...
func (api userChannelAPI) List(ctx context.Context, sid, userSid string) (UserChannelList, error) {
	var chanList UserChannelList
	data, err := api.client.Get(ctx, fmt.Sprintf("/Services/%s/Users/%s/Channels", sid, userSid))
	if err != nil {
		return chanList, err
	}
